...
gen() {
  ...
  [ -f "$OUT/vo${n}.wav" ] && { echo "L$n YA EXISTE"; return 0; }
  local resp id st
  ...
  if [ -z "$id" ]; then echo "L$n ERROR POST"; return 1; fi
    ...
    case "$st" in
      completed*)
        if [ -f "/mnt/480ssd/voicebox/data/generations/${id}.wav" ]; then
          cp "/mnt/480ssd/voicebox/data/generations/${id}.wav" "$OUT/vo${n}.wav"; echo "L$n OK $st"; return 0
        fi
        echo "L$n OK SIN WAV"; return 1;;
      failed*|error*) echo "L$n FAIL $st"; return 1;;
    esac
    sleep 4
  done
  echo "L$n TIMEOUT"; return 1
}
